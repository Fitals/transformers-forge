...
class OneFormerProcessor(ProcessorMixin):
    ...
    def __call__(self, images=None, task_inputs=None, segmentation_maps=None, **kwargs):
        ...
        if isinstance(task_inputs, list) and all(isinstance(task_input, str) for task_input in task_inputs):
            ...
            # Move task_inputs to the same device as pixel_values if available
            if "pixel_values" in encoded_inputs and hasattr(encoded_inputs["pixel_values"], "device"):
                encoded_inputs["task_inputs"] = encoded_inputs["task_inputs"].to(encoded_inputs["pixel_values"].device)
        else:
