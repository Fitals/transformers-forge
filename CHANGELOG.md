...
- **Flash Mode** — Режим ускоренного обучения (1.3-1.5x)
  - **ALAS (Adaptive Layer Activity Spectrum)** — динамическое управление backward pass
    - Спектр активности слоёв (0.3 - 1.0)
    - Интерполяция градиентов с учётом тренда
    - Стохастические решения на основе heat/stability слоя
  - **Sample Weighting** — адаптивное взвешивание примеров по loss
    - Низкий loss → сниженный вес (минимум 0.3)
    - Плавный sigmoid-переход между весами
  - **Conservative GCA** — консервативная экстраполяция градиентов
    - Порог когерентности 0.95
    - Максимум 2 виртуальных шага
    - Автоматический rollback при нестабильности
  ...
  trainer = Trainer(model=model, callbacks=[FlashModeCallback(config)])
  ```
...
- `loss_spike_threshold = 1.1` — авто-отключение при росте loss
- `warmup_steps = 100` — стабилизация перед активацией
...
- 14 новых тестов для Flash Mode

---

## [1.1.2] - 2025-12-19 — LR Finder Integration

### ✨ Новые возможности

- **Режим Find в Interactive Manager** — интеграция LR Finder в wizard
  - Введите 'Find' в меню настройки параметров
  - Автоматический подбор оптимального LR перед обучением
  - Показ результатов с подтверждением "Применить? Y/n"

### 📚 Документация

- Обновлена документация Interactive Manager с описанием режима Find
- Добавлена таблица режимов (Auto / Find / Manual)
