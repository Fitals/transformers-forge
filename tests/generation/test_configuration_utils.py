...
class GenerationConfigTest(unittest.TestCase):
    ...
    def test_explicitly_set_attrs_tracking(self):
        """Tests that _explicitly_set_attrs tracks which attributes were explicitly passed to __init__ (Issue #42762)."""
        # Default config has no explicitly set attrs (empty frozenset)
        default_config = GenerationConfig()
        self.assertEqual(default_config._explicitly_set_attrs, frozenset())

        # Config with explicit values should track them
        config_with_temp = GenerationConfig(temperature=1.0)  # even if it equals the default!
        self.assertIn("temperature", config_with_temp._explicitly_set_attrs)

        config_with_multiple = GenerationConfig(temperature=0.7, do_sample=True, top_k=40)
        self.assertEqual(config_with_multiple._explicitly_set_attrs, frozenset({"temperature", "do_sample", "top_k"}))

    def test_explicitly_set_attrs_survives_deepcopy(self):
        """Tests that _explicitly_set_attrs is preserved through deepcopy (Issue #42762)."""
        config = GenerationConfig(temperature=0.7, do_sample=True)
        copied_config = copy.deepcopy(config)

        self.assertEqual(config._explicitly_set_attrs, copied_config._explicitly_set_attrs)
        self.assertIn("temperature", copied_config._explicitly_set_attrs)
        self.assertIn("do_sample", copied_config._explicitly_set_attrs)

    def test_explicitly_set_attrs_not_in_dict_or_json(self):
        """Tests that _explicitly_set_attrs does not appear in to_dict() or to_json_string() (Issue #42762)."""
        config = GenerationConfig(temperature=0.7, do_sample=True)

        # Should not appear in to_dict()
        config_dict = config.to_dict()
        self.assertNotIn("_explicitly_set_attrs", config_dict)

        # Should not appear in JSON string
        json_string = config.to_json_string(ignore_metadata=False)
        self.assertNotIn("_explicitly_set_attrs", json_string)

        # Should also not appear when saved and loaded
        with tempfile.TemporaryDirectory() as tmp_dir:
            config.save_pretrained(tmp_dir)
            # Check the saved file doesn't contain _explicitly_set_attrs
            with open(os.path.join(tmp_dir, "generation_config.json"), "r") as f:
                saved_json = f.read()
            self.assertNotIn("_explicitly_set_attrs", saved_json)
