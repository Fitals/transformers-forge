...
**Ожидаемое улучшение:** +1-3% на eval метриках (для больших моделей)

#### Benchmark результаты

| Эксперимент | Результат | Статус |
|-------------|-----------|--------|
| Synthetic data (500 steps) | EMA отстаёт на -18% | ⚠️ Ожидаемо |
| LLM fine-tuning (10k+ steps) | Предстоит | 🔴 |

**⚠️ Важное наблюдение:**

Наш benchmark показал что EMA **не улучшает** результат на:
- Маленьких моделях (256 hidden size)
- Синтетических данных (нет шума)
- Коротком обучении (500 steps)

**Это ожидаемо и объясняется теорией:**

EMA накапливает историю весов. На начальных этапах веса плохие, и EMA "помнит" их. Польза EMA проявляется когда:

1. **Большие модели** — больше шума в градиентах
2. **Длинное обучение** — EMA успевает накопить хорошие веса
3. **Реальные данные** — есть overfitting который EMA сглаживает

**Когда использовать EMA:**

| Сценарий | Рекомендация |
|----------|--------------|
| Fine-tuning LLM 7B+ | ✅ Да, decay=0.999 |
| Diffusion models | ✅ Обязательно |
| Короткое обучение <1k steps | ❌ Не рекомендуется |
| Маленькие модели | ❌ Избыточно |
...
|--------|--------|----------|-------|
| EMA | ✅ Доказано | ⚠️ Не для малых моделей | ✅ Работают |
| Layer Freezing | ✅ Обосновано | 🔴 Предстоит | ✅ Работают |
| Training Presets | ✅ Best practices | 🔴 Предстоит | ✅ Работают |
| Training Monitor | ✅ Стандартные метрики | ✅ Проверено | ✅ Работают |
