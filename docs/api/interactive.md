...
5. **Настройка параметров** — Auto / Find / Manual
6. **Запуск обучения**
...
| Режим | Описание |
|-------|----------|
| **Auto** | Фиксированные preset-настройки (LR=2e-5) |
| **Find** | Автоматический подбор LR через LR Finder *(NEW!)* |
| **Manual** | Ручной выбор A/B/C пресетов для каждого параметра |

### Режим Find (LR Finder) *(v1.1.2)*

```
Режим настройки [Auto/Find/manual]: find

🔍 РЕЖИМ LR FINDER
   Автоматический подбор оптимального Learning Rate
   На основе метода Leslie Smith (2015)
   ⏱️ Займёт ~30-60 секунд

⏳ Загрузка модели...
⏳ Подготовка датасета...

🔍 LR Finder: поиск оптимального learning rate...
   Шаг 10/50: LR=1.26e-06, Loss=3.89
   Шаг 20/50: LR=1.58e-04, Loss=1.54
   ✅ Рекомендуемый LR: 2.30e-05

   Применить найденные настройки? [Y/n]: y
```
...
### v1.1.2
- **Режим Find** — интеграция LR Finder для автоматического подбора LR
